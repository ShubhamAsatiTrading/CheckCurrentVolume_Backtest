...
# Thread-safe rate limiting with monitoring
...
def fetch_data_in_chunks(kite, symbol: str, token: int, start_date: datetime.datetime, 
    ...
    while current_start < end_date:
        ...
        try:
            thread_id = str(threading.get_ident())[-4:]  # Last 4 digits of thread ID
            print(f"   📥 [T{thread_id}] Fetching {symbol}: {current_start.date()} to {chunk_end.date()} (Chunk {chunk_count}/{total_chunks})")
            
            # Thread-safe rate limiting with monitoring
            global _api_call_count, _error_count
            with _rate_limit_lock:
                ...
                try:
                    ...
                    # Monitor API response time for rate limiting detection
                    if api_time > 2.0:  # Slow response might indicate throttling
                        print(f"      [T{thread_id}] WARNING: Slow API response ({api_time:.2f}s) - possible throttling")
                    
                except Exception as api_error:
                    _error_count += 1
                    if "429" in str(api_error) or "Too Many Requests" in str(api_error):
                        print(f"      [T{thread_id}] RATE LIMIT HIT! Reduce max_workers or increase sleep time")
            ...
            if data:
                ...
                print(f"      [T{thread_id}] SUCCESS: Retrieved {len(data)} records in {api_time:.2f}s")
            else:
                print(f"      [T{thread_id}] WARNING: No data for this chunk")
            
        except Exception as e:
            thread_id = str(threading.get_ident())[-4:]  # Last 4 digits of thread ID
            print(f"   [T{thread_id}] ERROR: Error fetching chunk for {symbol} ({current_start.date()}): {e}")
    ...
    if all_data:
        thread_id = str(threading.get_ident())[-4:]  # Last 4 digits of thread ID
        print(f"   [T{thread_id}] DATA: Total records retrieved for {symbol}: {len(all_data)}")
...
# NEW: Thread worker functions for multithreading
def _process_full_download_worker(args):
    """Worker function for full download processing"""
    kite, instruments, symbol, start, interval, output_folder, symbol_num, total = args
    
    print(f"\n[THREAD] Processing {symbol} ({symbol_num}/{total}) - Full Download... [TID: {threading.get_ident()}]")
    ...
    try:
        ...
        print(f"[ERROR] Thread error for {symbol}: {e}")
        return symbol, False

def _process_update_worker(args):
    """Worker function for update processing"""
    kite, instruments, symbol, start, interval, output_folder, symbol_num, total = args
    
    print(f"\n[THREAD] Processing {symbol} ({symbol_num}/{total}) - Update... [TID: {threading.get_ident()}]")
...
def download_historical_data(
    ...
    """Download/update historical stock data with intelligent processing and multithreading"""
    ...
    print(f"\n[LOADING] Processing {total_to_process} symbols using {max_workers} threads...")
    ...
    # Process missing symbols (full download) with multithreading
    if symbol_categories['missing']:
        print(f"\n[MULTI-THREAD] Processing {len(symbol_categories['missing'])} missing symbols...")
        
        # Prepare arguments for thread pool
        missing_args = [
            (kite, instruments, symbol, start, interval, output_folder, i+1, len(symbol_categories['missing']))
            for i, symbol in enumerate(symbol_categories['missing'])
        ]
        
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_symbol = {
                executor.submit(_process_full_download_worker, args): args[2] 
    ...
    # Process existing symbols that need updates with multithreading
    if symbol_categories['existing_update_needed']:
        print(f"\n[MULTI-THREAD] Processing {len(symbol_categories['existing_update_needed'])} symbols for updates...")
        
        # Prepare arguments for thread pool
        update_args = [
            (kite, instruments, symbol, start, interval, output_folder, i+1, len(symbol_categories['existing_update_needed']))
            for i, symbol in enumerate(symbol_categories['existing_update_needed'])
        ]
        
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_symbol = {
                executor.submit(_process_update_worker, args): args[2] 
    ...
    # Calculate total time and API usage stats
    total_time = time.time() - start_time
    global _api_call_count, _error_count
    
    print(f"\n[COMPLETE] All processing complete in {total_time:.1f} seconds!")
    print(f"   • Processed {total_to_process} symbols with {max_workers} threads")
    print(f"   • Average: {total_time/total_to_process:.1f}s per symbol")
    
    # API usage safety report
    if _api_call_count > 0:
        error_rate = (_error_count / _api_call_count) * 100
        calls_per_second = _api_call_count / total_time
        
        print(f"\n[API SAFETY REPORT]")
        print(f"   • Total API calls: {_api_call_count}")
        print(f"   • Error rate: {error_rate:.1f}%")
        print(f"   • API calls/second: {calls_per_second:.2f}")
        
        # Safety recommendations
        if error_rate > 5:
            print(f"   ⚠️  HIGH ERROR RATE! Consider reducing max_workers to {max(1, max_workers-1)}")
        elif calls_per_second > 3:
            print(f"   ⚠️  HIGH API RATE! Consider reducing max_workers to {max(1, max_workers-1)}")
        elif error_rate == 0 and calls_per_second < 2:
            print(f"   ✅ SAFE: You could try max_workers={min(8, max_workers+1)} for better performance")
        else:
            print(f"   ✅ OPTIMAL: Current settings are safe and efficient")
...
if __name__ == "__main__":
    ...
    # download_historical_data(kite, interval='5minute', max_workers=4)  # For 5-minute data with 4 threads
    # download_historical_data(kite, interval='day', start=datetime.datetime(2021, 1, 1), max_workers=6)  # Custom start date with 6 threads
    pass